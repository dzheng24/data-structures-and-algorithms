...
const onlyOddChars = (str) => {
  // Solution code here...
  const arr = str.split('');
  const odd = arr.filter((value, index) => {
    if(index % 2 === 1){
      return value;
    }
  })
...
const unenrollBrook = (arr) => arr.map(eachClass => eachClass.filter(student => !student.includes('Brook')))
