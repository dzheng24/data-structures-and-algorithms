...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let mass = 0;
  arr.forEach(person => person.name === 'Luke Skywalker' ? mass = parseInt(person.mass) : 0);
  return arr.filter(person => parseInt(person.mass) > mass).map(result => result.name).join(' - ');
}
